...
const Profile = ({ navigation }) => {
  ...
  const storeData = async () => {
    ...
    try {
      ...
      await AsyncStorage.mergeItem(
        "@ll_user",
        JSON.stringify({ ...newUser, avatar: avatar })
      );
      await AsyncStorage.mergeItem(
        "@ll_users",
        JSON.stringify({ [user.email]: newUser })
      );
    } catch (error) {
  ...
  const logout = async () => {
    setIsLoading(true);
    try {
      await AsyncStorage.removeItem("@ll_user");
  ...
  const getData = async () => {
    try {
      ...
      if (storagedUser !== null) {
        ...
        if (JSON.parse(storagedUser).avatar) {
          setAvatar(JSON.parse(storagedUser).avatar);
        }
      } else {
        navigation.navigate("Onboarding");
      }
      ...
      <View style={css.content}>
        <ScrollView contentContainerStyle={{ paddingVertical: 20 }}>
          ...
          <View style={css.avatar}>
            ...
            <Button onPress={pickAvatar}>{avatar ? "Change" : "Add"}</Button>
          ...
          <Button appearance="secondary" onPress={logout}>
            Log out
          </Button>

          <View style={css.footer}>
            <Button onPress={getData} appearance="outline">
              Discard changes
            </Button>
